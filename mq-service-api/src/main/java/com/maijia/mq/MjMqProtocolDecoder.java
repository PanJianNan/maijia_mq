...
import com.maijia.mq.client.MqClientHandler;
...
public class MjMqProtocolDecoder extends LengthFieldBasedFrameDecoder {
    ...
    /** 头部信息的大小应该是 1byte lengthHigh + 1byte lengthLow = 2 */
...
////            in.resetReaderIndex();
//            throw new Exception("内容实际长度不符合协议声明的长度");
//        }
...
//        Object obj = HessianSerializerUtil.deserialize(data);
        //还是不要解析成对象返回的好，这样收到信息的一方可以获取这个协议的信息，如内存长度等，内容的反序列化交由接收方处理
        MjMqProtocol protocol = new MjMqProtocol(dataLengthHigh, dataLengthHigh, data);
